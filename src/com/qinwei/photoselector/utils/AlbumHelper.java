 ...
 * @created createTime: 2015-9-11 下午10:01:56
 * @version 1.0
 */
	...
	void buildImagesBucketList() {
				...
				if (cur.getString(photoPathIndex)
					...
					if (new File(path).length() > 50) {
						...
						if (bucket == null) {
							...
							bucket.imageList = new ArrayList<PhotoEntity>();
							bucket.bucketName = bucketName;
						}
						bucket.count++;
						PhotoEntity photoEntity = new PhotoEntity();
						photoEntity.imageId = _id;
						photoEntity.imagePath = path;
						photoEntity.thumbnailPath = thumbnailList.get(_id);
						bucket.imageList.add(photoEntity);
					}
				}
			} while (cur.moveToNext());
		}
		...
		while (itr.hasNext()) {
			...
			for (int i = 0; i < bucket.imageList.size(); ++i) {
				PhotoEntity image = bucket.imageList.get(i);
