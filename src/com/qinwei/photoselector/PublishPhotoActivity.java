...
import com.qinwei.photoselector.utils.FileUtils;
...
public class PublishPhotoActivity extends BasePublishPhotoActivity implements OnClickListener {
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			Toast.makeText(PublishPhotoActivity.this, "图片压缩完成!", 1000).show();
				...
				try {
					...
					for (PhotoEntity photo : photos) {
						...
						if (!new File(temp).exists() && file.length() > 100 * 1024) {
							temp = FileUtils.saveBitmap(photo.imagePath, temp);
						} else {
